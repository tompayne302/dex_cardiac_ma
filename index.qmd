...
priors_cont_metareg <- brms::prior(normal(0, 1), class = b, coef = "Intercept") +
            brms::prior(cauchy(0,0.5), class = sd) +
...
m.brm_del_duration_age <- update(m.brm_del_duration, formula. = ~ . - Intercept + age_60, newdata = IVdat_del_duration_age, prior = priors_cont_metareg)

IVdat_del_duration_dose <- IVdat_del_duration[!is.na(IVdat_del_duration$dex_dose),]
m.brm_del_duration_dose <- update(m.brm_del_duration, formula. = ~ . - Intercept + dex_dose, newdata = IVdat_del_duration_dose, prior = priors_cont_metareg)
...
m.brm_extub_time_age <- update(m.brm_del_duration, formula. = ~ . - Intercept + age_60, newdata = IVdat_extub_time_age, prior = priors_cont_metareg)

IVdat_extub_time_dose <- IVdat_extub_time[!is.na(IVdat_extub_time$dex_dose),]
m.brm_extub_time_dose <- update(m.brm_del_duration, formula. = ~ . - Intercept + dex_dose, newdata = IVdat_extub_time_dose, prior = priors_cont_metareg)
...
m.brm_hosp_stay_age <- update(m.brm_del_duration, formula. = ~ . - Intercept + age_60, newdata = IVdat_hosp_stay_age, prior = priors_cont_metareg)

IVdat_hosp_stay_dose <- IVdat_hosp_stay[!is.na(IVdat_hosp_stay$dex_dose),]
m.brm_hosp_stay_dose <- update(m.brm_del_duration, formula. = ~ . - Intercept + dex_dose, newdata = IVdat_hosp_stay_dose, prior = priors_cont_metareg)
...
m.brm_icu_stay_age <- update(m.brm_del_duration, formula. = ~ . - Intercept + age_60, newdata = IVdat_icu_stay_age, prior = priors_cont_metareg)

IVdat_icu_stay_dose <- IVdat_icu_stay[!is.na(IVdat_icu_stay$dex_dose),]
m.brm_icu_stay_dose <- update(m.brm_del_duration, formula. = ~ . - Intercept + dex_dose, newdata = IVdat_icu_stay_dose, prior = priors_cont_metareg)
...
- Added a section for the average treatment effect (ATE) from DECADE in @tbl-ate and @fig-ecdfs-arr.
- Changed @tbl-arr to calculate risk differences directly from logistic regression using `marginaleffects` (rather than converting using the OR). Results are similar.
- Changed @tbl-benefit-probs-all and all other NNT tables to report absolute risk reductions rather than NNTs. See [Frank Harrell's issues](https://discourse.datamethods.org/t/problems-with-nnt/195) with the NNT. They have convinced me to stop using it (thanks Ben).
- Changed @fig-freq-funnel to be a contour-enhanced funnel plot rather than normal funnel plot.
- All incorrect references to 'publication bias' changed to 'small study effects'. See [this link](https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/pub-bias.html) for details.
- Priors specified and justified for the primary analysis and metaregression.
- I had made an error in the fully Bayesian sequential analysis in @tbl-fully-bayes-sequential. I was mistakenly putting our prior for $\tau$ on $\tau^2$ rather than $\tau$. I have changed this and added a longer explanation of how I created the new model in `rjags`. It now suggests stopping after 15 studies (including studies at high RoB), which is more congruent with the semi-Bayesian analysis. 
- Added a summary section (at the top) for our Bayesian reanalysis of DECADE.
- Added plots of our priors in @fig-prior-plot-effect and @fig-prior-plot-hetero. 


# Introduction

Before diving into this I would read [Kruschke and Liddell](https://link.springer.com/article/10.3758/s13423-016-1221-4) if you're not across the whole frequentist vs. Bayesian debate.
...
We can show our $\tau \sim HalfCauchy(0.5)$ prior grapically. Somewhat arbitrarily, traditional cutoffs for heterogeneity are: Low ($\tau < 0.1$), reasonable ($0.1 < \tau < 0.5$), fairly high ($0.5 < \tau < 1.0$), and fairly extreme ($\tau > 1.0$). We can present the prior probabilities of each level of heterogeneity graphically, as shown in @fig-prior-plot-hetero below.
...
I also present our primary priors for 𝝁 and 𝛕, $N(0, 0.82)$ and $HalfCauchy(0.5)$, respectively.
...
arr_2_Rt_overall <- Rc_overall - 0.02
arr_2_or_overall <- (arr_2_Rt_overall*(Rc_overall - 1))/(Rc_overall*(arr_2_Rt_overall - 1))

arr_4_Rt_overall <- Rc_overall - 0.04
arr_4_or_overall <- (arr_4_Rt_overall*(Rc_overall - 1))/(Rc_overall*(arr_4_Rt_overall - 1))

arr_6_Rt_overall <- Rc_overall - 0.06
arr_6_or_overall <- (arr_6_Rt_overall*(Rc_overall - 1))/(Rc_overall*(arr_6_Rt_overall - 1))

arr_8_Rt_overall <- Rc_overall - 0.08
arr_8_or_overall <- (arr_8_Rt_overall*(Rc_overall - 1))/(Rc_overall*(arr_8_Rt_overall - 1))

probs_overall = 
  m.brm |> 
  tidy_draws() |> 
  summarise(any_benefit_overall = 100*mean(b_Intercept < log(1)),
            rrr_arr_50_overall = 100*mean(b_Intercept < log(arr_2_or_overall)),
            rrr_arr_4_overall = 100*mean(b_Intercept < log(arr_4_or_overall)),
            rrr_arr_6_overall = 100*mean(b_Intercept < log(arr_6_or_overall)),
            rrr_arr_8_overall = 100*mean(b_Intercept < log(arr_8_or_overall)))

# First, age below 60
Rc_age_below_60 <- median(IVdat_del[IVdat_del$age_60=="No" &
                                               !is.na(IVdat_del$age_60),]$control_del_rate)
arr_2_Rt_age_below_60 <- Rc_age_below_60 - 0.02
arr_2_or_age_below_60 <- arr_2_Rt_age_below_60*(Rc_age_below_60 - 1)/Rc_age_below_60*(arr_2_Rt_age_below_60 - 1)

arr_4_Rt_age_below_60 <- Rc_age_below_60 - 0.04
arr_4_or_age_below_60 <- arr_4_Rt_age_below_60*(Rc_age_below_60 - 1)/Rc_age_below_60*(arr_4_Rt_age_below_60 - 1)

arr_6_Rt_age_below_60 <- Rc_age_below_60 - 0.06
arr_6_or_age_below_60 <- arr_6_Rt_age_below_60*(Rc_age_below_60 - 1)/Rc_age_below_60*(arr_6_Rt_age_below_60 - 1)

arr_8_Rt_age_below_60 <- Rc_age_below_60 - 0.08
arr_8_or_age_below_60 <- arr_8_Rt_age_below_60*(Rc_age_below_60 - 1)/Rc_age_below_60*(arr_8_Rt_age_below_60 - 1)

probs_age_below_60 = 
  spread_draws(m.brm_age, b_age_60No) |> 
  summarise(any_benefit_age_below_60 = 100*mean(b_age_60No < log(1)),
            rrr_arr_2_age_below_60 = 100*mean(b_age_60No < log(arr_2_or_age_below_60)),
            rrr_arr_4_age_below_60 = 100*mean(b_age_60No < log(arr_4_or_age_below_60)),
            rrr_arr_6_age_below_60 = 100*mean(b_age_60No < log(arr_6_or_age_below_60)),
            rrr_arr_8_age_below_60 = 100*mean(b_age_60No < log(arr_8_or_age_below_60)))

# then, age above 60
Rc_age_above_60 <- median(IVdat_del[IVdat_del$age_60=="Yes" &
                                               !is.na(IVdat_del$age_60),]$control_del_rate)
arr_2_Rt_age_above_60 <- Rc_age_above_60 - 0.02
arr_2_or_age_above_60 <- arr_2_Rt_age_above_60*(Rc_age_above_60 - 1)/Rc_age_above_60*(arr_2_Rt_age_above_60 - 1)

arr_4_Rt_age_above_60 <- Rc_age_above_60 - 0.04
arr_4_or_age_above_60 <- arr_4_Rt_age_above_60*(Rc_age_above_60 - 1)/Rc_age_above_60*(arr_4_Rt_age_above_60 - 1)

arr_6_Rt_age_above_60 <- Rc_age_above_60 - 0.06
arr_6_or_age_above_60 <- arr_6_Rt_age_above_60*(Rc_age_above_60 - 1)/Rc_age_above_60*(arr_6_Rt_age_above_60 - 1)

arr_8_Rt_age_above_60 <- Rc_age_above_60 - 0.08
arr_8_or_age_above_60 <- arr_8_Rt_age_above_60*(Rc_age_above_60 - 1)/Rc_age_above_60*(arr_8_Rt_age_above_60 - 1)

probs_age_above_60 = 
  spread_draws(m.brm_age, b_age_60Yes) |> 
  summarise(any_benefit_age_above_60 = 100*mean(b_age_60Yes < log(1)),
            rrr_arr_2_age_above_60 = 100*mean(b_age_60Yes < log(arr_2_or_age_above_60)),
            rrr_arr_4_age_above_60 = 100*mean(b_age_60Yes < log(arr_4_or_age_above_60)),
            rrr_arr_6_age_above_60 = 100*mean(b_age_60Yes < log(arr_6_or_age_above_60)),
            rrr_arr_8_age_above_60 = 100*mean(b_age_60Yes < log(arr_8_or_age_above_60)))
...
arr_2_Rt_high_dose <- Rc_high_dose - 0.02
arr_2_or_high_dose <- arr_2_Rt_high_dose*(Rc_high_dose - 1)/Rc_high_dose*(arr_2_Rt_high_dose - 1)

arr_4_Rt_high_dose <- Rc_high_dose - 0.04
arr_4_or_high_dose <- arr_4_Rt_high_dose*(Rc_high_dose - 1)/Rc_high_dose*(arr_4_Rt_high_dose - 1)

arr_6_Rt_high_dose <- Rc_high_dose - 0.06
arr_6_or_high_dose <- arr_6_Rt_high_dose*(Rc_high_dose - 1)/Rc_high_dose*(arr_6_Rt_high_dose - 1)

arr_8_Rt_high_dose <- Rc_high_dose - 0.08
arr_8_or_high_dose <- arr_8_Rt_high_dose*(Rc_high_dose - 1)/Rc_high_dose*(arr_8_Rt_high_dose - 1)

probs_high_dose = 
  spread_draws(m.brm_dose, b_dex_doseHigh) |> 
  summarise(any_benefit_high_dose = 100*mean(b_dex_doseHigh < log(1)),
            rrr_arr_2_high_dose = 100*mean(b_dex_doseHigh < log(arr_2_or_high_dose)),
            rrr_arr_4_high_dose = 100*mean(b_dex_doseHigh < log(arr_4_or_high_dose)),
            rrr_arr_6_high_dose = 100*mean(b_dex_doseHigh < log(arr_6_or_high_dose)),
            rrr_arr_8_high_dose = 100*mean(b_dex_doseHigh < log(arr_8_or_high_dose)))

# Low dose
Rc_low_dose <- median(IVdat_del[IVdat_del$dex_dose=="Low" &
                                               !is.na(IVdat_del$dex_dose),]$control_del_rate)
arr_2_Rt_low_dose <- Rc_low_dose - 0.02
arr_2_or_low_dose <- arr_2_Rt_low_dose*(Rc_low_dose - 1)/Rc_low_dose*(arr_2_Rt_low_dose - 1)

arr_4_Rt_low_dose <- Rc_low_dose - 0.04
arr_4_or_low_dose <- arr_4_Rt_low_dose*(Rc_low_dose - 1)/Rc_low_dose*(arr_4_Rt_low_dose - 1)

arr_6_Rt_low_dose <- Rc_low_dose - 0.06
arr_6_or_low_dose <- arr_6_Rt_low_dose*(Rc_low_dose - 1)/Rc_low_dose*(arr_6_Rt_low_dose - 1)

arr_8_Rt_low_dose <- Rc_low_dose - 0.08
arr_8_or_low_dose <- arr_8_Rt_low_dose*(Rc_low_dose - 1)/Rc_low_dose*(arr_8_Rt_low_dose - 1)

probs_low_dose = 
  spread_draws(m.brm_dose, b_dex_doseLow) |> 
  summarise(any_benefit_low_dose = 100*mean(b_dex_doseLow < log(1)),
            rrr_arr_2_low_dose = 100*mean(b_dex_doseLow < log(arr_2_or_low_dose)),
            rrr_arr_4_low_dose = 100*mean(b_dex_doseLow < log(arr_4_or_low_dose)),
            rrr_arr_6_low_dose = 100*mean(b_dex_doseLow < log(arr_6_or_low_dose)),
            rrr_arr_8_low_dose = 100*mean(b_dex_doseLow < log(arr_8_or_low_dose)))
...
prob_tbl1 <- as.data.frame(rbind("Overall" = c("Overall", "Overall", Rc_overall, probs_overall),
  ...
  mutate_at(vars(3:8), ~sprintf("%.1f", .))

colnames(prob_tbl1) <- c("Subgroup", "Group", "Median control group rate (%)", "Probability of any benefit (%)", "Probability of ARR = 2 (%)",
                        "Probability of ARR = 4 (%)", "Probability of ARR = 6 (%)", "Probability of ARR = 8 (%)")
...
#| tbl-cap: Probability of various ARRs, excluding studies at high risk of bias.

Rc_overall <- median(IVdat_del_low_rob$control_del_rate)
...
# First, age below 60
Rc_age_below_60 <- median(IVdat_del_low_rob[IVdat_del_low_rob$age_60=="No" &
                                               !is.na(IVdat_del_low_rob$age_60),]$control_del_rate)
arr_2_Rt_age_below_60 <- Rc_age_below_60 - 0.02
arr_2_or_age_below_60 <- arr_2_Rt_age_below_60*(Rc_age_below_60 - 1)/Rc_age_below_60*(arr_2_Rt_age_below_60 - 1)
...
# then, age above 60
Rc_age_above_60 <- median(IVdat_del_low_rob[IVdat_del_low_rob$age_60=="Yes" &
                                               !is.na(IVdat_del_low_rob$age_60),]$control_del_rate)
arr_2_Rt_age_above_60 <- Rc_age_above_60 - 0.02
arr_2_or_age_above_60 <- arr_2_Rt_age_above_60*(Rc_age_above_60 - 1)/Rc_age_above_60*(arr_2_Rt_age_above_60 - 1)
...
## Now dose
# High dose
Rc_high_dose <- median(IVdat_del_low_rob[IVdat_del_low_rob$dex_dose=="High" &
                                               !is.na(IVdat_del_low_rob$dex_dose),]$control_del_rate)
...
# Low dose
Rc_low_dose <- median(IVdat_del_low_rob[IVdat_del_low_rob$dex_dose=="Low" &
                                               !is.na(IVdat_del_low_rob$dex_dose),]$control_del_rate)
arr_2_Rt_low_dose <- Rc_low_dose - 0.02
arr_2_or_low_dose <- arr_2_Rt_low_dose*(Rc_low_dose - 1)/Rc_low_dose*(arr_2_Rt_low_dose - 1)
...
$$\hat{\theta} \sim Normal(\theta, \frac{1}{\sigma^2})$$
$$\theta \sim Normal(\mu, \frac{1}{\tau^2})$$
$$\mu \sim Normal(0, \frac{1}{0.82^2})$$
$$\tau^2 \sim HalfNormal(0, \frac{1}{10^2})$$

Below in @tbl-fully-bayes-sequential we present the results of the fully Bayesian sequential analysis. As can be seen, this method suggests stopping for benefit after 15 studies when all studies are included, but the conclusion is uncertain when excluding studies at high risk of bias.

```{r}
#| label: tbl-fully-bayes-sequential
#| tbl-cap: Fully Bayesian sequential method.
...
write("model {for (i in 1:n) { 
      ...
      theta ~ dnorm(mu, prec)
      tsq ~ dnorm(0, 1/100) T(0,)}", "BayesMA.bug")
...
Bayesian.seqMA <- function(yi, vi, delta, 
  ...
  i <- 1; result <- "Uncertain"; diag <- c()
                      ...
                      BayesMA.jags <- jags.model("BayesMA.bug", n.chains=chains, 
                                                 ...
                                                 inits=list(theta=runif(1, -2, 2), tsq = runif(1, 0, 0.5)))
...
# Create function to calculate number of studies
count_na <- function(dataframe) {
  return(dataframe %>% nrow())
}

dat_all <- Bayesian.seqMA(yi=IVdat_del[,132], vi=IVdat_del[,133], delta=-0.18)

df_all <- data.frame(cbind("subgroup" = "All studies", "outcome" = dat_all$result, "no_studies" = dat_all$studies, 
                           "total_no_studies" = count_na(IVdat_del),
                       "mu" = exp(dat_all$theta), "mu_ci_lb" = exp(dat_all$theta.hpd[[1]]), 
                       "mu_ci_ub" = exp(dat_all$theta.hpd[[2]]), "tau_sq" = sqrt(dat_all$tau.sq),
                       "tau_sq_lb" = sqrt(dat_all$tau.sq.hpd[[1]]), "tau_sq_ub" = sqrt(dat_all$tau.sq.hpd[[2]]))) %>%
   mutate(across(5:10, as.numeric),
          across(where(is.numeric), ~sprintf("%.2f", .)),
          mu_conf = paste0(mu, " (", mu_ci_lb, ", ", mu_ci_ub, ")"),
          tau_conf = paste0(tau_sq, " (", tau_sq_lb, ", ", tau_sq_ub, ")")) %>%
  dplyr::select(subgroup, outcome, no_studies, total_no_studies, mu_conf, tau_conf)

## Repeat same excluding low RoB studies
dat_low_rob <- Bayesian.seqMA(yi=IVdat_del_low_rob[,132], vi=IVdat_del_low_rob[,133], delta=-0.18)

df_low_rob <- data.frame(cbind("subgroup" = "Excluding studies at high RoB", "outcome" = dat_low_rob$result, "no_studies" = dat_low_rob$studies, 
                               "total_no_studies" = count_na(IVdat_del_low_rob),
                       "mu" = exp(dat_low_rob$theta), "mu_ci_lb" = exp(dat_low_rob$theta.hpd[[1]]), 
                       "mu_ci_ub" = exp(dat_low_rob$theta.hpd[[2]]), "tau_sq" = sqrt(dat_low_rob$tau.sq),
                       "tau_sq_lb" = sqrt(dat_low_rob$tau.sq.hpd[[1]]), "tau_sq_ub" = sqrt(dat_low_rob$tau.sq.hpd[[2]]))) %>%
   mutate(across(5:10, as.numeric),
          across(where(is.numeric), ~sprintf("%.2f", .)),
          mu_conf = paste0(mu, " (", mu_ci_lb, ", ", mu_ci_ub, ")"),
          tau_conf = paste0(tau_sq, " (", tau_sq_lb, ", ", tau_sq_ub, ")")) %>%
...
rbind(df_all, df_low_rob) %>%
  rename("Subgroup" = subgroup, "Conclusion" = outcome, "Number of studies to reach conclusion" = no_studies,
         "Total number of studies in subgroup" = total_no_studies,
         "Median odds ratio (95%CrI) at stopping" = mu_conf, "τ (95% CrI) at stopping" = tau_conf) %>%
  gt() %>%
    tab_style(
    style = list(
      cell_fill(color = "white"),
...
As can be seen below in @tbl-semibayes-all, this method suggests the benefit is uncertain with the current data. The slight discordance with the fully Bayesian model is more stringent criteria for stopping in the Semi-Bayesian model.
...
all <- ggplot(aes(exp(b_Intercept), 
  ...
  geom_segment(aes(x = 3.5, y = 2, xend = 1.5, yend = 2),
             color = "black",
             size = 0.2,
             arrow = arrow(length = unit(0.02, "npc"), type = "closed")) +             
  annotate("text", x = 3, y = 3,  label = "Benefit certain\n(Fully Bayesian)", colour = "darkgreen") +
  theme_light() +
...
3.  Calculate the probabilities of various absolute risk reductions (from the DECADE trial) using the priors in step 1.

4.  Present and compare empirical cumulative distribution function (ECDF) curves for A) the meta-analysis including all studies, B) the meta-analysis excluding the DECADE trial, C) the DECADE trial alone, and D) the shrinkage estimate of the DECADE trial.

5.  Calculate the average treatment effect (ATE) (in the form of a risk difference and credible interval) from the DECADE trial using the priors in step 1.
...
With all that in mind, I have the following data-derived priors for DECADE's results, which are shown further down in @fig-priors-plots-all:
...
-   Strongly skeptical: $Normal(0.27, 0.186)$. Equivalent to an RCT enrolling 1000 people and showing a 31% increase in the odds of delirium ($MCID × 1.5$) with dexmedetomidine.
...
-   Strongly optimistic: $Normal(-0.27, 0.206)$. Equivalent to an RCT enrolling 1000 people and showing a 24% reduction in the odds of delirium ($MCID × 1.5$) with dexmedetomidine.
...
full_weight_ma_priors <- brms::prior(normal(-0.516, 0.170), class = b, coef = "treatmentdexmedetomidine") +
...
half_weight_ma_priors <- brms::prior(normal(-0.516, 0.170*2), class = b, coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
quarter_weight_ma_priors <- brms::prior(normal(-0.516, 0.170*4), class = b, coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
very_skeptical_priors <- brms::prior(normal(0.27, 0.1856227), coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
skeptical_priors <- brms::prior(normal(0.18, 0.2666484), coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
optimistic_priors <- brms::prior(normal(-0.18, 0.2869203), coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
very_optimistic_priors <- brms::prior(normal(-0.27, 0.2059696), coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)
...
```{r}
#| label: fig-priors-plots-low-rob
#| fig-width: 12
#| fig-height: 10
#| cache: TRUE
#| results: 'hide'
...
#| errors: FALSE
#| fig-cap: |
#|   Plots of the distributions of the reference and meta-analysis priors, excluding studies at high risk of bias.
...
## First create our MA priors
## start with the fully weighted MA prior
median_mu_excl_decade <- m.brm_excluding_decade_low_rob %>%
  spread_draws(b_Intercept) %>%
  median_qi() %>%
  dplyr::select(b_Intercept)
#  -0.410

median_tau_excl_decade <- m.brm_excluding_decade_low_rob %>%
  spread_draws(sd_author1__Intercept) %>%
  median_qi() %>%
  dplyr::select(sd_author1__Intercept)
# 0.137
...
full_weight_ma_priors <- brms::prior(normal(-0.410, 0.137), coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)

ma_full_weight.brm_low_rob <- brm(delirium ~ 1 + treatment,
             data = df,
             family = bernoulli(link = "logit"),
             prior = full_weight_ma_priors,
              iter = 4000,
...
ma_full_weight_draws <- draws_fn(ma_full_weight.brm_low_rob)

ma_full_weight_forest_draws <- forest_draws_fn(ma_full_weight.brm_low_rob, "MA (100% weight)")
ma_full_weight_benefit <- benefit_fn(ma_full_weight.brm_low_rob)

## Now for the 50% weight (get this by multiplying SD by 2)

half_weight_ma_priors <- brms::prior(normal(-0.410, 0.137*2), class = b, coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)

ma_half_weight.brm_low_rob <- update(ma_full_weight.brm_low_rob, prior = half_weight_ma_priors)

ma_half_weight_draws <- draws_fn(ma_half_weight.brm_low_rob)
ma_half_weight_forest_draws <- forest_draws_fn(ma_half_weight.brm_low_rob, "MA (50% weight)")
ma_half_weight_benefit <- benefit_fn(ma_half_weight.brm_low_rob)


## Now for the 25% weight (get this by multiplying SD by 4)

quarter_weight_ma_priors <- brms::prior(normal(-0.410, 0.137*4), class = b, coef = "treatmentdexmedetomidine") +
                brms::prior(normal(0,1), class = Intercept)

ma_quarter_weight.brm_low_rob <- update(ma_full_weight.brm_low_rob, prior = quarter_weight_ma_priors)

ma_quarter_weight_draws <- draws_fn(ma_quarter_weight.brm_low_rob)
ma_quarter_weight_forest_draws <- forest_draws_fn(ma_quarter_weight.brm_low_rob, "MA (25% weight)")
ma_quarter_weight_benefit <- benefit_fn(ma_quarter_weight.brm_low_rob)
...
vague.brm <- brm(delirium ~ 1 + treatment,
             data = df,
             family = bernoulli(link = "logit"),
             prior = vague_priors,
              iter = 4000,
...
very_skeptical.brm <- update(vague.brm, prior = very_skeptical_priors)
...
skeptical.brm <- update(vague.brm, prior = skeptical_priors)
...
neutral.brm <- update(vague.brm, prior = neutral_priors)
...
optimistic.brm <- update(vague.brm, prior = optimistic_priors)
optimistic_draws <- draws_fn(optimistic.brm)
optimistic_forest_draws <- forest_draws_fn(optimistic.brm, "Optimistic")
optimistic_benefit <- benefit_fn(optimistic.brm)
...
very_optimistic.brm <- update(vague.brm, prior = very_optimistic_priors)
...
ma_priors <- ggplot(data = data.frame(x = c(-10, 10)), aes(x)) +
  geom_vline(xintercept = 1, color = "black", size = 1) +
  stat_slab(aes(xdist = exp(distributional::dist_normal(mean = -0.410, sd = 0.137)), color = "metafull", y = 1),
            fill = NA, slab_linewidth = 0.9, inherit.aes = FALSE) +
  stat_slab(aes(xdist = exp(distributional::dist_normal(mean = -0.410, sd = (0.137*2))), color = "metahalf", y = 1),
            fill = NA, slab_linewidth = 0.9, inherit.aes = FALSE) +
  stat_slab(aes(xdist = exp(distributional::dist_normal(mean = -0.410, sd = (0.137*4))), color = "metaquarter", y = 1),
            fill = NA, slab_linewidth = 0.9, inherit.aes = FALSE) +
  scale_x_log10(breaks = c(0.1, 0.25, 0.5, 1, 2, 4, 7), expand = c(0, 0)) +
  coord_cartesian(xlim = c(0.1, 7), ylim = c(1, 2)) +
  ggdist::scale_thickness_shared() +
...
tabdat_pre <- data.frame(subgroup = factor(c("Vague", "Very skeptical", 
                     ...
                     decade_posterior_mean = IVdat_del$yi[IVdat_del$author1 == "Turan 2020"],
                     decade_posterior_sd = IVdat_del$sei[IVdat_del$author1 == "Turan 2020"],
                     prior_mean = c(0, 0.27, 0.18, 0, -0.18, -0.27, -0.410, -0.410, -0.410),
                     prior_sd = c(10, se_very_skeptical, se_skeptical, se_neutral,
                                  se_optimistic, se_very_optimistic, 0.137, 0.137*2, 0.137*4),
                     posterior_estimate = c(vague_draws$estimate, very_skeptical_draws$estimate, skeptical_draws$estimate,
                                            neutral_draws$estimate, optimistic_draws$estimate, very_optimistic_draws$estimate, 
                                            ma_full_weight_draws$estimate, ma_half_weight_draws$estimate, ma_quarter_weight_draws$estimate),
                     p_benefit = c(vague_benefit$prob_benefit, very_skeptical_benefit$prob_benefit, skeptical_benefit$prob_benefit,
...
All ARRs in @tbl-arr below use `avg_comparisons()` function from the `marginaleffects` package to obtain risk differences as the measure of effect.
...
probs_fn <- function(brm) {
      brm %>% 
       avg_comparisons() %>%
       posterior_draws() %>%
       summarise(arr_8 = 100*mean(draw < -0.08),
            arr_4 = 100*mean(draw < -0.04),
            arr_2 = 100*mean(draw < -0.02),
            arr_below_0 = 100*mean(draw < 0),
            arr_above_0 = 100*mean(draw > 0),
            ari_2 = 100*mean(draw > 0.02),
            ari_4 = 100*mean(draw > 0.04),
            ari_8 = 100*mean(draw > 0.08))
}
...
probs <- rbind(probs_fn(vague.brm), probs_fn(very_skeptical.brm), probs_fn(skeptical.brm),
               ...
               probs_fn(ma_full_weight.brm_low_rob), probs_fn(ma_half_weight.brm_low_rob), 
               probs_fn(ma_quarter_weight.brm_low_rob)) %>%
  mutate(across(where(is.numeric), ~sprintf("%.1f", .)))

tabdat <- data.frame(prior = factor(c("Vague", "Very skeptical", 
                                "Skeptical", "Neutral", "Optimistic", 
                                "Very optimistic", "MA (100% weight)", "MA (50% weight)", "MA (25% weight)", 
                                "MA (100% weight)", "MA (50% weight)", "MA (25% weight)")),
                     Group = c("Reference priors", "Reference priors","Reference priors","Reference priors",
                               "Reference priors","Reference priors",
                               "Meta-analysis priors (all studies)", "Meta-analysis priors (all studies)", 
                               "Meta-analysis priors (all studies)",
                               "Meta-analysis priors (excluding high RoB)", "Meta-analysis priors (excluding high RoB)", 
...
colnames(tabdat) <- c("Prior", "Group", "Prior equivalent", "Rationale for prior", "ARR >8%", "ARR >4%",
...
## Now to make the plot combining the 4 curves of interest
decade_posterior_mean <-  IVdat_del_low_rob$yi[IVdat_del_low_rob$author1 == "Turan 2020"]
decade_posterior_sd <- IVdat_del_low_rob$sei[IVdat_del_low_rob$author1 == "Turan 2020"]
...
However, [Harrell](https://www.fharrell.com/post/marg/) does caution us that:
...
dat_ma <- rbind(arr_fn(ma_full_weight.brm, "MA (100% weight)"), arr_fn(ma_half_weight.brm, "MA (50% weight)"), arr_fn(ma_quarter_weight.brm, "MA (25% weight)"),
                arr_fn(ma_full_weight.brm_low_rob, "MA (100% weight)"), arr_fn(ma_half_weight.brm_low_rob, "MA (50% weight)"), 
                arr_fn(ma_quarter_weight.brm_low_rob, "MA (25% weight)"))
                
dat_reference <- rbind(arr_fn(vague.brm, "Vague"), arr_fn(very_skeptical.brm, "Very sceptical"), arr_fn(skeptical.brm, "Sceptical"),
             arr_fn(neutral.brm, "Neutral"), arr_fn(optimistic.brm, "Optimistic"), arr_fn(very_optimistic.brm, "Very optimistic"))
...
In @fig-ecdfs-arr, the posterior probability of continuous risk differences from the DECADE trial is shown for each prior. The posterior density of the draws are also shown, for each respective prior.
...
#| fig-height: 10
#| fig-cap: |
#|   Empirical cumulative distribution function (ECDF) plots for the ARR.
...
probs <- rbind(probs_fn(vague.brm, "Vague"), probs_fn(very_skeptical.brm, "Very sceptical"), probs_fn(skeptical.brm, "Sceptical"),
               probs_fn(neutral.brm, "Neutral"), probs_fn(optimistic.brm, "Optimistic"), probs_fn(very_optimistic.brm, "Very optimistic"),
               probs_fn(ma_full_weight.brm, "MA (100% weight)"), probs_fn(ma_half_weight.brm, "MA (50% weight)"), probs_fn(ma_quarter_weight.brm, "MA (25% weight)"),
               probs_fn(ma_full_weight.brm_low_rob, "MA (100% weight), excluding high RoB"), probs_fn(ma_half_weight.brm_low_rob, "MA (50% weight), excluding high RoB"), 
               probs_fn(ma_quarter_weight.brm_low_rob, "MA (25% weight), excluding high RoB")) 

ecdfs <- ggplot(aes(x = draw, color = prior), data = probs) +
  stat_ecdf(geom = "step") +
  scale_y_continuous(name = "Probability that mean risk difference < x", limits = c(0, 1), breaks = seq(0, 1, 0.1), labels = percent_format(),
                     sec.axis = sec_axis(~ 1 - ., name = "Probability that mean risk difference > x",
                                          breaks = seq(0, 1, 0.1), labels = percent_format())) +
  geom_vline(xintercept = 0) +
  scale_x_continuous(breaks = c(-7.5, -5, -2.5, 0, 2.5, 5, 7.5, 10, 12.5), expand = c(0, 0)) +
  coord_cartesian(xlim = c(-7.5, 12.5)) +
  theme_light() +
  theme(
    legend.position = c(0.95, 0.6),  
    legend.justification = c(1, 1),
    legend.background = element_rect(fill = "lightgrey", color = "black")) +
  xlab(label = "Absolute risk reduction (%)") +
  scale_color_manual(values = RColorBrewer::brewer.pal(12, "Set3"),
                     breaks = c("Vague", "Very skeptical", 
                                "Skeptical", "Neutral", "Optimistic", 
                                "Very optimistic", "MA (100% weight)", "MA (50% weight)", "MA (25% weight)", 
                                "MA (100% weight), excluding high RoB", "MA (50% weight), excluding high RoB", "MA (25% weight), excluding high RoB"),
                     labels = c("Vague", "Very skeptical", 
                                "Skeptical", "Neutral", "Optimistic", 
                                "Very optimistic", "MA (100% weight)", "MA (50% weight)", "MA (25% weight)", 
                                "MA (100% weight), excluding high RoB", "MA (50% weight), excluding high RoB", "MA (25% weight), excluding high RoB"),
...
ma_curves <- ggplot(aes(x = draw, color = prior), data = probs) +
  stat_slab(fill = NA) +
  scale_x_continuous(breaks = c(-7.5, -5, -2.5, 0, 2.5, 5, 7.5, 10, 12.5), expand = c(0, 0)) +
  coord_cartesian(xlim = c(-7.5, 12.5)) +
  ggdist::scale_thickness_shared() +
  geom_vline(xintercept=0) +
  annotate("text", x = -5, y = 0.9, label = "Favours\ndexmedetomidine", size = 4) +
  annotate("text", x = 5, y = 0.9, label = "Favours\ncontrol",  size = 4) +
  theme_light() +
  theme(
    axis.text.y = element_blank(),
...
ari_2_Rt_overall_bradycardia <- Rc_overall_bradycardia + 0.02
ari_2_or_overall_bradycardia <- (ari_2_Rt_overall_bradycardia*(Rc_overall_bradycardia - 1))/(Rc_overall_bradycardia*(ari_2_Rt_overall_bradycardia - 1))

ari_4_Rt_overall_bradycardia <- Rc_overall_bradycardia + 0.04
ari_4_or_overall_bradycardia <- (ari_4_Rt_overall_bradycardia*(Rc_overall_bradycardia - 1))/(Rc_overall_bradycardia*(ari_4_Rt_overall_bradycardia - 1))

ari_6_Rt_overall_bradycardia <- Rc_overall_bradycardia + 0.06
ari_6_or_overall_bradycardia <- (ari_6_Rt_overall_bradycardia*(Rc_overall_bradycardia - 1))/(Rc_overall_bradycardia*(ari_6_Rt_overall_bradycardia - 1))

ari_8_Rt_overall_bradycardia <- Rc_overall_bradycardia + 0.08
ari_8_or_overall_bradycardia <- (ari_8_Rt_overall_bradycardia*(Rc_overall_bradycardia - 1))/(Rc_overall_bradycardia*(ari_8_Rt_overall_bradycardia - 1))

probs_overall_bradycardia = 
  m.brm_bradycardia |> 
  tidy_draws() |> 
  summarise(any_benefit_overall_bradycardia = 100*mean(b_Intercept > log(1)),
            rrr_ari_2_overall_bradycardia = 100*mean(b_Intercept > log(ari_2_or_overall_bradycardia)),
            rrr_ari_4_overall_bradycardia = 100*mean(b_Intercept > log(ari_4_or_overall_bradycardia)),
            rrr_ari_6_overall_bradycardia = 100*mean(b_Intercept > log(ari_6_or_overall_bradycardia)),
            rrr_ari_8_overall_bradycardia = 100*mean(b_Intercept > log(ari_8_or_overall_bradycardia)))
...
ari_2_Rt_age_below_60_bradycardia <- Rc_age_below_60_bradycardia + 0.02
ari_2_or_age_below_60_bradycardia <- (ari_2_Rt_age_below_60_bradycardia*(Rc_age_below_60_bradycardia - 1))/(Rc_age_below_60_bradycardia*(ari_2_Rt_age_below_60_bradycardia - 1))

ari_4_Rt_age_below_60_bradycardia <- Rc_age_below_60_bradycardia + 0.04
ari_4_or_age_below_60_bradycardia <- (ari_4_Rt_age_below_60_bradycardia*(Rc_age_below_60_bradycardia - 1))/(Rc_age_below_60_bradycardia*(ari_4_Rt_age_below_60_bradycardia - 1))

ari_6_Rt_age_below_60_bradycardia <- Rc_age_below_60_bradycardia + 0.06
ari_6_or_age_below_60_bradycardia <- (ari_6_Rt_age_below_60_bradycardia*(Rc_age_below_60_bradycardia - 1))/(Rc_age_below_60_bradycardia*(ari_6_Rt_age_below_60_bradycardia - 1))

ari_8_Rt_age_below_60_bradycardia <- Rc_age_below_60_bradycardia + 0.08
ari_8_or_age_below_60_bradycardia <- (ari_8_Rt_age_below_60_bradycardia*(Rc_age_below_60_bradycardia - 1))/(Rc_age_below_60_bradycardia*(ari_8_Rt_age_below_60_bradycardia - 1))
...
probs_age_below_60_bradycardia = 
  ...
  summarise(any_benefit_age_below_60_bradycardia = 100*mean(b_age_60No > log(1)),
            rrr_ari_2_age_below_60_bradycardia = 100*mean(b_age_60No > log(ari_2_or_age_below_60_bradycardia)),
            rrr_ari_4_age_below_60_bradycardia = 100*mean(b_age_60No > log(ari_4_or_age_below_60_bradycardia)),
            rrr_ari_6_age_below_60_bradycardia = 100*mean(b_age_60No > log(ari_6_or_age_below_60_bradycardia)),
            rrr_ari_8_age_below_60_bradycardia = 100*mean(b_age_60No > log(ari_8_or_age_below_60_bradycardia)))

# Then age above 60
Rc_age_above_60_bradycardia <- median(IVdat_bradycardia[IVdat_bradycardia$age_60=="Yes" &
                                               !is.na(IVdat_bradycardia$age_60),]$control_bradycardia_rate)
ari_2_Rt_age_above_60_bradycardia <- Rc_age_above_60_bradycardia + 0.02
ari_2_or_age_above_60_bradycardia <- (ari_2_Rt_age_above_60_bradycardia*(Rc_age_above_60_bradycardia - 1))/(Rc_age_above_60_bradycardia*(ari_2_Rt_age_above_60_bradycardia - 1))

ari_4_Rt_age_above_60_bradycardia <- Rc_age_above_60_bradycardia + 0.04
ari_4_or_age_above_60_bradycardia <- (ari_4_Rt_age_above_60_bradycardia*(Rc_age_above_60_bradycardia - 1))/(Rc_age_above_60_bradycardia*(ari_4_Rt_age_above_60_bradycardia - 1))

ari_6_Rt_age_above_60_bradycardia <- Rc_age_above_60_bradycardia + 0.06
ari_6_or_age_above_60_bradycardia <- (ari_6_Rt_age_above_60_bradycardia*(Rc_age_above_60_bradycardia - 1))/(Rc_age_above_60_bradycardia*(ari_6_Rt_age_above_60_bradycardia - 1))

ari_8_Rt_age_above_60_bradycardia <- Rc_age_above_60_bradycardia + 0.08
ari_8_or_age_above_60_bradycardia <- (ari_8_Rt_age_above_60_bradycardia*(Rc_age_above_60_bradycardia - 1))/(Rc_age_above_60_bradycardia*(ari_8_Rt_age_above_60_bradycardia - 1))
...
probs_age_above_60_bradycardia = 
  ...
  summarise(any_benefit_age_above_60_bradycardia = 100*mean(b_age_60Yes > log(1)),
            rrr_ari_2_age_above_60_bradycardia = 100*mean(b_age_60Yes > log(ari_2_or_age_above_60_bradycardia)),
            rrr_ari_4_age_above_60_bradycardia = 100*mean(b_age_60Yes > log(ari_4_or_age_above_60_bradycardia)),
            rrr_ari_6_age_above_60_bradycardia = 100*mean(b_age_60Yes > log(ari_6_or_age_above_60_bradycardia)),
            rrr_ari_8_age_above_60_bradycardia = 100*mean(b_age_60Yes > log(ari_8_or_age_above_60_bradycardia)))
...
ari_2_Rt_high_dose_bradycardia <- Rc_high_dose_bradycardia + 0.02
ari_2_or_high_dose_bradycardia <- (ari_2_Rt_high_dose_bradycardia*(Rc_high_dose_bradycardia - 1))/(Rc_high_dose_bradycardia*(ari_2_Rt_high_dose_bradycardia - 1))

ari_4_Rt_high_dose_bradycardia <- Rc_high_dose_bradycardia + 0.04
ari_4_or_high_dose_bradycardia <- (ari_4_Rt_high_dose_bradycardia*(Rc_high_dose_bradycardia - 1))/(Rc_high_dose_bradycardia*(ari_4_Rt_high_dose_bradycardia - 1))

ari_6_Rt_high_dose_bradycardia <- Rc_high_dose_bradycardia + 0.06
ari_6_or_high_dose_bradycardia <- (ari_6_Rt_high_dose_bradycardia*(Rc_high_dose_bradycardia - 1))/(Rc_high_dose_bradycardia*(ari_6_Rt_high_dose_bradycardia - 1))

ari_8_Rt_high_dose_bradycardia <- Rc_high_dose_bradycardia + 0.08
ari_8_or_high_dose_bradycardia <- (ari_8_Rt_high_dose_bradycardia*(Rc_high_dose_bradycardia - 1))/(Rc_high_dose_bradycardia*(ari_8_Rt_high_dose_bradycardia - 1))
...
probs_high_dose_bradycardia = 
  ...
  summarise(any_benefit_high_dose_bradycardia = 100*mean(b_dex_doseHigh > log(1)),
            rrr_ari_2_high_dose_bradycardia = 100*mean(b_dex_doseHigh > log(ari_2_or_high_dose_bradycardia)),
            rrr_ari_4_high_dose_bradycardia = 100*mean(b_dex_doseHigh > log(ari_4_or_high_dose_bradycardia)),
            rrr_ari_6_high_dose_bradycardia = 100*mean(b_dex_doseHigh > log(ari_6_or_high_dose_bradycardia)),
            rrr_ari_8_high_dose_bradycardia = 100*mean(b_dex_doseHigh > log(ari_8_or_high_dose_bradycardia)))

# Low dose
Rc_low_dose_bradycardia <- median(IVdat_bradycardia[IVdat_bradycardia$dex_dose=="Low" &
                                               !is.na(IVdat_bradycardia$dex_dose),]$control_bradycardia_rate)
ari_2_Rt_low_dose_bradycardia <- Rc_low_dose_bradycardia + 0.02
ari_2_or_low_dose_bradycardia <- (ari_2_Rt_low_dose_bradycardia*(Rc_low_dose_bradycardia - 1))/(Rc_low_dose_bradycardia*(ari_2_Rt_low_dose_bradycardia - 1))

ari_4_Rt_low_dose_bradycardia <- Rc_low_dose_bradycardia + 0.04
ari_4_or_low_dose_bradycardia <- (ari_4_Rt_low_dose_bradycardia*(Rc_low_dose_bradycardia - 1))/(Rc_low_dose_bradycardia*(ari_4_Rt_low_dose_bradycardia - 1))

ari_6_Rt_low_dose_bradycardia <- Rc_low_dose_bradycardia + 0.06
ari_6_or_low_dose_bradycardia <- (ari_6_Rt_low_dose_bradycardia*(Rc_low_dose_bradycardia - 1))/(Rc_low_dose_bradycardia*(ari_6_Rt_low_dose_bradycardia - 1))

ari_8_Rt_low_dose_bradycardia <- Rc_low_dose_bradycardia + 0.08
ari_8_or_low_dose_bradycardia <- (ari_8_Rt_low_dose_bradycardia*(Rc_low_dose_bradycardia - 1))/(Rc_low_dose_bradycardia*(ari_8_Rt_low_dose_bradycardia - 1))
...
probs_low_dose_bradycardia = 
  ...
  summarise(any_benefit_low_dose_bradycardia = 100*mean(b_dex_doseLow > log(1)),
            rrr_ari_2_low_dose_bradycardia = 100*mean(b_dex_doseLow > log(ari_2_or_low_dose_bradycardia)),
            rrr_ari_4_low_dose_bradycardia = 100*mean(b_dex_doseLow > log(ari_4_or_low_dose_bradycardia)),
            rrr_ari_6_low_dose_bradycardia = 100*mean(b_dex_doseLow > log(ari_6_or_low_dose_bradycardia)),
            rrr_ari_8_low_dose_bradycardia = 100*mean(b_dex_doseLow > log(ari_8_or_low_dose_bradycardia)))
...
prob_tbl1 <- as.data.frame(rbind("Overall" = c("Overall",  Rc_overall_bradycardia, probs_overall_bradycardia),
  ...
  rename(`Subgroup` = V1,
          `median_rate` = V2) %>%
  mutate(`median_rate` = `median_rate` * 100) %>%
  mutate_at(vars(2:7), ~sprintf("%.1f", .))

colnames(prob_tbl1) <- c("Subgroup", "Median control group rate (%)", "Probability of any harm (%)", "Probability of ARI = 2 (%)",
                        "Probability of ARI = 4 (%)", "Probability of ARI = 6 (%)", "Probability of ARI = 8 (%)")
...
ari_2_Rt_overall_hypotension <- Rc_overall_hypotension + 0.02
ari_2_or_overall_hypotension <- (ari_2_Rt_overall_hypotension*(Rc_overall_hypotension - 1))/(Rc_overall_hypotension*(ari_2_Rt_overall_hypotension - 1))

ari_4_Rt_overall_hypotension <- Rc_overall_hypotension + 0.04
ari_4_or_overall_hypotension <- (ari_4_Rt_overall_hypotension*(Rc_overall_hypotension - 1))/(Rc_overall_hypotension*(ari_4_Rt_overall_hypotension - 1))

ari_6_Rt_overall_hypotension <- Rc_overall_hypotension + 0.06
ari_6_or_overall_hypotension <- (ari_6_Rt_overall_hypotension*(Rc_overall_hypotension - 1))/(Rc_overall_hypotension*(ari_6_Rt_overall_hypotension - 1))

ari_8_Rt_overall_hypotension <- Rc_overall_hypotension + 0.08
ari_8_or_overall_hypotension <- (ari_8_Rt_overall_hypotension*(Rc_overall_hypotension - 1))/(Rc_overall_hypotension*(ari_8_Rt_overall_hypotension - 1))

probs_overall_hypotension = 
  m.brm_hypotension |> 
  tidy_draws() |> 
  summarise(any_benefit_overall_hypotension = 100*mean(b_Intercept > log(1)),
            rrr_ari_2_overall_hypotension = 100*mean(b_Intercept > log(ari_2_or_overall_hypotension)),
            rrr_ari_4_overall_hypotension = 100*mean(b_Intercept > log(ari_4_or_overall_hypotension)),
            rrr_ari_6_overall_hypotension = 100*mean(b_Intercept > log(ari_6_or_overall_hypotension)),
            rrr_ari_8_overall_hypotension = 100*mean(b_Intercept > log(ari_8_or_overall_hypotension)))
...
ari_2_Rt_age_below_60_hypotension <- Rc_age_below_60_hypotension + 0.02
ari_2_or_age_below_60_hypotension <- (ari_2_Rt_age_below_60_hypotension*(Rc_age_below_60_hypotension - 1))/(Rc_age_below_60_hypotension*(ari_2_Rt_age_below_60_hypotension - 1))

ari_4_Rt_age_below_60_hypotension <- Rc_age_below_60_hypotension + 0.04
ari_4_or_age_below_60_hypotension <- (ari_4_Rt_age_below_60_hypotension*(Rc_age_below_60_hypotension - 1))/(Rc_age_below_60_hypotension*(ari_4_Rt_age_below_60_hypotension - 1))

ari_6_Rt_age_below_60_hypotension <- Rc_age_below_60_hypotension + 0.06
ari_6_or_age_below_60_hypotension <- (ari_6_Rt_age_below_60_hypotension*(Rc_age_below_60_hypotension - 1))/(Rc_age_below_60_hypotension*(ari_6_Rt_age_below_60_hypotension - 1))

ari_8_Rt_age_below_60_hypotension <- Rc_age_below_60_hypotension + 0.08
ari_8_or_age_below_60_hypotension <- (ari_8_Rt_age_below_60_hypotension*(Rc_age_below_60_hypotension - 1))/(Rc_age_below_60_hypotension*(ari_8_Rt_age_below_60_hypotension - 1))
...
probs_age_below_60_hypotension = 
  ...
  summarise(any_benefit_age_below_60_hypotension = 100*mean(b_age_60No > log(1)),
            rrr_ari_2_age_below_60_hypotension = 100*mean(b_age_60No > log(ari_2_or_age_below_60_hypotension)),
            rrr_ari_4_age_below_60_hypotension = 100*mean(b_age_60No > log(ari_4_or_age_below_60_hypotension)),
            rrr_ari_6_age_below_60_hypotension = 100*mean(b_age_60No > log(ari_6_or_age_below_60_hypotension)),
            rrr_ari_8_age_below_60_hypotension = 100*mean(b_age_60No > log(ari_8_or_age_below_60_hypotension)))

# Then age above 60
Rc_age_above_60_hypotension <- median(IVdat_hypotension[IVdat_hypotension$age_60=="Yes" &
                                               !is.na(IVdat_hypotension$age_60),]$control_hypotension_rate)
ari_2_Rt_age_above_60_hypotension <- Rc_age_above_60_hypotension + 0.02
ari_2_or_age_above_60_hypotension <- (ari_2_Rt_age_above_60_hypotension*(Rc_age_above_60_hypotension - 1))/(Rc_age_above_60_hypotension*(ari_2_Rt_age_above_60_hypotension - 1))

ari_4_Rt_age_above_60_hypotension <- Rc_age_above_60_hypotension + 0.04
ari_4_or_age_above_60_hypotension <- (ari_4_Rt_age_above_60_hypotension*(Rc_age_above_60_hypotension - 1))/(Rc_age_above_60_hypotension*(ari_4_Rt_age_above_60_hypotension - 1))

ari_6_Rt_age_above_60_hypotension <- Rc_age_above_60_hypotension + 0.06
ari_6_or_age_above_60_hypotension <- (ari_6_Rt_age_above_60_hypotension*(Rc_age_above_60_hypotension - 1))/(Rc_age_above_60_hypotension*(ari_6_Rt_age_above_60_hypotension - 1))

ari_8_Rt_age_above_60_hypotension <- Rc_age_above_60_hypotension + 0.08
ari_8_or_age_above_60_hypotension <- (ari_8_Rt_age_above_60_hypotension*(Rc_age_above_60_hypotension - 1))/(Rc_age_above_60_hypotension*(ari_8_Rt_age_above_60_hypotension - 1))
...
probs_age_above_60_hypotension = 
  ...
  summarise(any_benefit_age_above_60_hypotension = 100*mean(b_age_60Yes > log(1)),
            rrr_ari_2_age_above_60_hypotension = 100*mean(b_age_60Yes > log(ari_2_or_age_above_60_hypotension)),
            rrr_ari_4_age_above_60_hypotension = 100*mean(b_age_60Yes > log(ari_4_or_age_above_60_hypotension)),
            rrr_ari_6_age_above_60_hypotension = 100*mean(b_age_60Yes > log(ari_6_or_age_above_60_hypotension)),
            rrr_ari_8_age_above_60_hypotension = 100*mean(b_age_60Yes > log(ari_8_or_age_above_60_hypotension)))
...
ari_2_Rt_high_dose_hypotension <- Rc_high_dose_hypotension + 0.02
ari_2_or_high_dose_hypotension <- (ari_2_Rt_high_dose_hypotension*(Rc_high_dose_hypotension - 1))/(Rc_high_dose_hypotension*(ari_2_Rt_high_dose_hypotension - 1))

ari_4_Rt_high_dose_hypotension <- Rc_high_dose_hypotension + 0.04
ari_4_or_high_dose_hypotension <- (ari_4_Rt_high_dose_hypotension*(Rc_high_dose_hypotension - 1))/(Rc_high_dose_hypotension*(ari_4_Rt_high_dose_hypotension - 1))

ari_6_Rt_high_dose_hypotension <- Rc_high_dose_hypotension + 0.06
ari_6_or_high_dose_hypotension <- (ari_6_Rt_high_dose_hypotension*(Rc_high_dose_hypotension - 1))/(Rc_high_dose_hypotension*(ari_6_Rt_high_dose_hypotension - 1))

ari_8_Rt_high_dose_hypotension <- Rc_high_dose_hypotension + 0.08
ari_8_or_high_dose_hypotension <- (ari_8_Rt_high_dose_hypotension*(Rc_high_dose_hypotension - 1))/(Rc_high_dose_hypotension*(ari_8_Rt_high_dose_hypotension - 1))
...
probs_high_dose_hypotension = 
  ...
  summarise(any_benefit_high_dose_hypotension = 100*mean(b_dex_doseHigh > log(1)),
            rrr_ari_2_high_dose_hypotension = 100*mean(b_dex_doseHigh > log(ari_2_or_high_dose_hypotension)),
            rrr_ari_4_high_dose_hypotension = 100*mean(b_dex_doseHigh > log(ari_4_or_high_dose_hypotension)),
            rrr_ari_6_high_dose_hypotension = 100*mean(b_dex_doseHigh > log(ari_6_or_high_dose_hypotension)),
           rrr_ari_8_high_dose_hypotension = 100*mean(b_dex_doseHigh > log(ari_8_or_high_dose_hypotension)))

# Low dose
Rc_low_dose_hypotension <- median(IVdat_hypotension[IVdat_hypotension$dex_dose=="Low" &
                                               !is.na(IVdat_hypotension$dex_dose),]$control_hypotension_rate)
ari_2_Rt_low_dose_hypotension <- Rc_low_dose_hypotension + 0.02
ari_2_or_low_dose_hypotension <- (ari_2_Rt_low_dose_hypotension*(Rc_low_dose_hypotension - 1))/(Rc_low_dose_hypotension*(ari_2_Rt_low_dose_hypotension - 1))

ari_4_Rt_low_dose_hypotension <- Rc_low_dose_hypotension + 0.04
ari_4_or_low_dose_hypotension <- (ari_4_Rt_low_dose_hypotension*(Rc_low_dose_hypotension - 1))/(Rc_low_dose_hypotension*(ari_4_Rt_low_dose_hypotension - 1))

ari_6_Rt_low_dose_hypotension <- Rc_low_dose_hypotension + 0.06
ari_6_or_low_dose_hypotension <- (ari_6_Rt_low_dose_hypotension*(Rc_low_dose_hypotension - 1))/(Rc_low_dose_hypotension*(ari_6_Rt_low_dose_hypotension - 1))

ari_8_Rt_low_dose_hypotension <- Rc_low_dose_hypotension + 0.08
ari_8_or_low_dose_hypotension <- (ari_8_Rt_low_dose_hypotension*(Rc_low_dose_hypotension - 1))/(Rc_low_dose_hypotension*(ari_8_Rt_low_dose_hypotension - 1))
...
probs_low_dose_hypotension = 
  ...
  summarise(any_benefit_low_dose_hypotension = 100*mean(b_dex_doseLow > log(1)),
            rrr_ari_2_low_dose_hypotension = 100*mean(b_dex_doseLow > log(ari_2_or_low_dose_hypotension)),
            rrr_ari_4_low_dose_hypotension = 100*mean(b_dex_doseLow > log(ari_4_or_low_dose_hypotension)),
            rrr_ari_6_low_dose_hypotension = 100*mean(b_dex_doseLow > log(ari_6_or_low_dose_hypotension)),
            rrr_ari_8_low_dose_hypotension = 100*mean(b_dex_doseLow > log(ari_8_or_low_dose_hypotension)))
...
colnames(prob_tbl2) <- c("Subgroup", "Median control group rate (%)", "Probability of any harm (%)", "Probability of ARI = 2 (%)",
                        "Probability of ARI = 4 (%)", "Probability of ARI = 6 (%)", "Probability of ARI = 8 (%)")
...
arr_2_Rt_overall_arrhythmia <- Rc_overall_arrhythmia - 0.02
arr_2_or_overall_arrhythmia <- (arr_2_Rt_overall_arrhythmia*(Rc_overall_arrhythmia - 1))/(Rc_overall_arrhythmia*(arr_2_Rt_overall_arrhythmia - 1))

arr_4_Rt_overall_arrhythmia <- Rc_overall_arrhythmia - 0.04
arr_4_or_overall_arrhythmia <- (arr_4_Rt_overall_arrhythmia*(Rc_overall_arrhythmia - 1))/(Rc_overall_arrhythmia*(arr_4_Rt_overall_arrhythmia - 1))

arr_6_Rt_overall_arrhythmia <- Rc_overall_arrhythmia - 0.06
arr_6_or_overall_arrhythmia <- (arr_6_Rt_overall_arrhythmia*(Rc_overall_arrhythmia - 1))/(Rc_overall_arrhythmia*(arr_6_Rt_overall_arrhythmia - 1))

arr_8_Rt_overall_arrhythmia <- Rc_overall_arrhythmia - 0.08
arr_8_or_overall_arrhythmia <- (arr_8_Rt_overall_arrhythmia*(Rc_overall_arrhythmia - 1))/(Rc_overall_arrhythmia*(arr_8_Rt_overall_arrhythmia - 1))
...
probs_overall_arrhythmia = 
  ...
  summarise(any_benefit_overall_arrhythmia = 100*mean(b_Intercept < log(1)),
            rrr_arr_2_overall_arrhythmia = 100*mean(b_Intercept < log(arr_2_or_overall_arrhythmia)),
            rrr_arr_4_overall_arrhythmia = 100*mean(b_Intercept < log(arr_4_or_overall_arrhythmia)),
            rrr_arr_6_overall_arrhythmia = 100*mean(b_Intercept < log(arr_6_or_overall_arrhythmia)),
            rrr_arr_8_overall_arrhythmia = 100*mean(b_Intercept < log(arr_8_or_overall_arrhythmia)))
...
arr_2_Rt_age_below_60_arrhythmia <- Rc_age_below_60_arrhythmia - 0.02
arr_2_or_age_below_60_arrhythmia <- (arr_2_Rt_age_below_60_arrhythmia*(Rc_age_below_60_arrhythmia - 1))/(Rc_age_below_60_arrhythmia*(arr_2_Rt_age_below_60_arrhythmia - 1))

arr_4_Rt_age_below_60_arrhythmia <- Rc_age_below_60_arrhythmia - 0.04
arr_4_or_age_below_60_arrhythmia <- (arr_4_Rt_age_below_60_arrhythmia*(Rc_age_below_60_arrhythmia - 1))/(Rc_age_below_60_arrhythmia*(arr_4_Rt_age_below_60_arrhythmia - 1))

arr_6_Rt_age_below_60_arrhythmia <- Rc_age_below_60_arrhythmia - 0.06
arr_6_or_age_below_60_arrhythmia <- (arr_6_Rt_age_below_60_arrhythmia*(Rc_age_below_60_arrhythmia - 1))/(Rc_age_below_60_arrhythmia*(arr_6_Rt_age_below_60_arrhythmia - 1))

arr_8_Rt_age_below_60_arrhythmia <- Rc_age_below_60_arrhythmia - 0.08
arr_8_or_age_below_60_arrhythmia <- (arr_8_Rt_age_below_60_arrhythmia*(Rc_age_below_60_arrhythmia - 1))/(Rc_age_below_60_arrhythmia*(arr_8_Rt_age_below_60_arrhythmia - 1))
...
probs_age_below_60_arrhythmia = 
  ...
  summarise(any_benefit_age_below_60_arrhythmia = 100*mean(b_age_60No < log(1)),
            rrr_arr_2_age_below_60_arrhythmia = 100*mean(b_age_60No < log(arr_2_or_age_below_60_arrhythmia)),
            rrr_arr_4_age_below_60_arrhythmia = 100*mean(b_age_60No < log(arr_4_or_age_below_60_arrhythmia)),
            rrr_arr_6_age_below_60_arrhythmia = 100*mean(b_age_60No < log(arr_6_or_age_below_60_arrhythmia)),
            rrr_arr_8_age_below_60_arrhythmia = 100*mean(b_age_60No < log(arr_8_or_age_below_60_arrhythmia)))

# Then age above 60
Rc_age_above_60_arrhythmia <- median(IVdat_arrhythmia[IVdat_arrhythmia$age_60=="Yes" &
                                               !is.na(IVdat_arrhythmia$age_60),]$control_arrhythmia_rate)
arr_2_Rt_age_above_60_arrhythmia <- Rc_age_above_60_arrhythmia - 0.02
arr_2_or_age_above_60_arrhythmia <- (arr_2_Rt_age_above_60_arrhythmia*(Rc_age_above_60_arrhythmia - 1))/(Rc_age_above_60_arrhythmia*(arr_2_Rt_age_above_60_arrhythmia - 1))

arr_4_Rt_age_above_60_arrhythmia <- Rc_age_above_60_arrhythmia - 0.04
arr_4_or_age_above_60_arrhythmia <- (arr_4_Rt_age_above_60_arrhythmia*(Rc_age_above_60_arrhythmia - 1))/(Rc_age_above_60_arrhythmia*(arr_4_Rt_age_above_60_arrhythmia - 1))

arr_6_Rt_age_above_60_arrhythmia <- Rc_age_above_60_arrhythmia - 0.06
arr_6_or_age_above_60_arrhythmia <- (arr_6_Rt_age_above_60_arrhythmia*(Rc_age_above_60_arrhythmia - 1))/(Rc_age_above_60_arrhythmia*(arr_6_Rt_age_above_60_arrhythmia - 1))

arr_8_Rt_age_above_60_arrhythmia <- Rc_age_above_60_arrhythmia - 0.08
arr_8_or_age_above_60_arrhythmia <- (arr_8_Rt_age_above_60_arrhythmia*(Rc_age_above_60_arrhythmia - 1))/(Rc_age_above_60_arrhythmia*(arr_8_Rt_age_above_60_arrhythmia - 1))
...
probs_age_above_60_arrhythmia = 
  ...
  summarise(any_benefit_age_above_60_arrhythmia = 100*mean(b_age_60Yes < log(1)),
            rrr_arr_2_age_above_60_arrhythmia = 100*mean(b_age_60Yes < log(arr_2_or_age_above_60_arrhythmia)),
            rrr_arr_4_age_above_60_arrhythmia = 100*mean(b_age_60Yes < log(arr_4_or_age_above_60_arrhythmia)),
            rrr_arr_6_age_above_60_arrhythmia = 100*mean(b_age_60Yes < log(arr_6_or_age_above_60_arrhythmia)),
            rrr_arr_8_age_above_60_arrhythmia = 100*mean(b_age_60Yes < log(arr_8_or_age_above_60_arrhythmia)))
...
arr_2_Rt_high_dose_arrhythmia <- Rc_high_dose_arrhythmia - 0.02
arr_2_or_high_dose_arrhythmia <- (arr_2_Rt_high_dose_arrhythmia*(Rc_high_dose_arrhythmia - 1))/(Rc_high_dose_arrhythmia*(arr_2_Rt_high_dose_arrhythmia - 1))

arr_4_Rt_high_dose_arrhythmia <- Rc_high_dose_arrhythmia - 0.04
arr_4_or_high_dose_arrhythmia <- (arr_4_Rt_high_dose_arrhythmia*(Rc_high_dose_arrhythmia - 1))/(Rc_high_dose_arrhythmia*(arr_4_Rt_high_dose_arrhythmia - 1))

arr_6_Rt_high_dose_arrhythmia <- Rc_high_dose_arrhythmia - 0.06
arr_6_or_high_dose_arrhythmia <- (arr_6_Rt_high_dose_arrhythmia*(Rc_high_dose_arrhythmia - 1))/(Rc_high_dose_arrhythmia*(arr_6_Rt_high_dose_arrhythmia - 1))

arr_8_Rt_high_dose_arrhythmia <- Rc_high_dose_arrhythmia - 0.08
arr_8_or_high_dose_arrhythmia <- (arr_8_Rt_high_dose_arrhythmia*(Rc_high_dose_arrhythmia - 1))/(Rc_high_dose_arrhythmia*(arr_8_Rt_high_dose_arrhythmia - 1))
...
probs_high_dose_arrhythmia = 
  ...
  summarise(any_benefit_high_dose_arrhythmia = 100*mean(b_dex_doseHigh < log(1)),
            rrr_arr_2_high_dose_arrhythmia = 100*mean(b_dex_doseHigh < log(arr_2_or_high_dose_arrhythmia)),
            rrr_arr_4_high_dose_arrhythmia = 100*mean(b_dex_doseHigh < log(arr_4_or_high_dose_arrhythmia)),
            rrr_arr_6_high_dose_arrhythmia = 100*mean(b_dex_doseHigh < log(arr_6_or_high_dose_arrhythmia)),
            rrr_arr_8_high_dose_arrhythmia = 100*mean(b_dex_doseHigh < log(arr_8_or_high_dose_arrhythmia)))

# Low dose
Rc_low_dose_arrhythmia <- median(IVdat_arrhythmia[IVdat_arrhythmia$dex_dose=="Low" &
                                               !is.na(IVdat_arrhythmia$dex_dose),]$control_arrhythmia_rate)
arr_2_Rt_low_dose_arrhythmia <- Rc_low_dose_arrhythmia - 0.02
arr_2_or_low_dose_arrhythmia <- (arr_2_Rt_low_dose_arrhythmia*(Rc_low_dose_arrhythmia - 1))/(Rc_low_dose_arrhythmia*(arr_2_Rt_low_dose_arrhythmia - 1))

arr_4_Rt_low_dose_arrhythmia <- Rc_low_dose_arrhythmia - 0.04
arr_4_or_low_dose_arrhythmia <- (arr_4_Rt_low_dose_arrhythmia*(Rc_low_dose_arrhythmia - 1))/(Rc_low_dose_arrhythmia*(arr_4_Rt_low_dose_arrhythmia - 1))

arr_6_Rt_low_dose_arrhythmia <- Rc_low_dose_arrhythmia - 0.06
arr_6_or_low_dose_arrhythmia <- (arr_6_Rt_low_dose_arrhythmia*(Rc_low_dose_arrhythmia - 1))/(Rc_low_dose_arrhythmia*(arr_6_Rt_low_dose_arrhythmia - 1))

arr_8_Rt_low_dose_arrhythmia <- Rc_low_dose_arrhythmia - 0.08
arr_8_or_low_dose_arrhythmia <- (arr_8_Rt_low_dose_arrhythmia*(Rc_low_dose_arrhythmia - 1))/(Rc_low_dose_arrhythmia*(arr_8_Rt_low_dose_arrhythmia - 1))
...
probs_low_dose_arrhythmia = 
  ...
  summarise(any_benefit_low_dose_arrhythmia = 100*mean(b_dex_doseLow < log(1)),
            rrr_arr_2_low_dose_arrhythmia = 100*mean(b_dex_doseLow < log(arr_2_or_low_dose_arrhythmia)),
            rrr_arr_4_low_dose_arrhythmia = 100*mean(b_dex_doseLow < log(arr_4_or_low_dose_arrhythmia)),
            rrr_arr_6_low_dose_arrhythmia = 100*mean(b_dex_doseLow < log(arr_6_or_low_dose_arrhythmia)),
            rrr_arr_8_low_dose_arrhythmia = 100*mean(b_dex_doseLow < log(arr_8_or_low_dose_arrhythmia)))
...
colnames(prob_tbl1) <- c("Subgroup", "Median control group rate (%)", "Probability of any benefit (%)", "Probability of ARR = 2 (%)",
                        "Probability of ARR = 4 (%)", "Probability of ARR = 6 (%)", "Probability of ARR = 8 (%)")
...
arr_2_Rt_overall_mortality <- Rc_overall_mortality - 0.02
arr_2_or_overall_mortality <- (arr_2_Rt_overall_mortality*(Rc_overall_mortality - 1))/(Rc_overall_mortality*(arr_2_Rt_overall_mortality - 1))

arr_4_Rt_overall_mortality <- Rc_overall_mortality - 0.04
arr_4_or_overall_mortality <- (arr_4_Rt_overall_mortality*(Rc_overall_mortality - 1))/(Rc_overall_mortality*(arr_4_Rt_overall_mortality - 1))

arr_6_Rt_overall_mortality <- Rc_overall_mortality - 0.06
arr_6_or_overall_mortality <- (arr_6_Rt_overall_mortality*(Rc_overall_mortality - 1))/(Rc_overall_mortality*(arr_6_Rt_overall_mortality - 1))

arr_8_Rt_overall_mortality <- Rc_overall_mortality - 0.08
arr_8_or_overall_mortality <- (arr_8_Rt_overall_mortality*(Rc_overall_mortality - 1))/(Rc_overall_mortality*(arr_8_Rt_overall_mortality - 1))

probs_overall_mortality = 
  m.brm_mortality |> 
  tidy_draws() |> 
  summarise(any_benefit_overall_mortality = 100*mean(b_Intercept < log(1)),
            rrr_arr_2_overall_mortality = 100*mean(b_Intercept < log(arr_2_or_overall_mortality)),
            rrr_arr_4_overall_mortality = 100*mean(b_Intercept < log(arr_4_or_overall_mortality)),
            rrr_arr_6_overall_mortality = 100*mean(b_Intercept < log(arr_6_or_overall_mortality)),
            rrr_arr_8_overall_mortality = 100*mean(b_Intercept < log(arr_8_or_overall_mortality)))
...
arr_2_Rt_age_below_60_mortality <- Rc_age_below_60_mortality - 0.02
arr_2_or_age_below_60_mortality <- (arr_2_Rt_age_below_60_mortality*(Rc_age_below_60_mortality - 1))/(Rc_age_below_60_mortality*(arr_2_Rt_age_below_60_mortality - 1))

arr_4_Rt_age_below_60_mortality <- Rc_age_below_60_mortality - 0.04
arr_4_or_age_below_60_mortality <- (arr_4_Rt_age_below_60_mortality*(Rc_age_below_60_mortality - 1))/(Rc_age_below_60_mortality*(arr_4_Rt_age_below_60_mortality - 1))

arr_6_Rt_age_below_60_mortality <- Rc_age_below_60_mortality - 0.06
arr_6_or_age_below_60_mortality <- (arr_6_Rt_age_below_60_mortality*(Rc_age_below_60_mortality - 1))/(Rc_age_below_60_mortality*(arr_6_Rt_age_below_60_mortality - 1))

arr_8_Rt_age_below_60_mortality <- Rc_age_below_60_mortality - 0.08
arr_8_or_age_below_60_mortality <- (arr_8_Rt_age_below_60_mortality*(Rc_age_below_60_mortality - 1))/(Rc_age_below_60_mortality*(arr_8_Rt_age_below_60_mortality - 1))
...
probs_age_below_60_mortality = 
  ...
  summarise(any_benefit_age_below_60_mortality = 100*mean(b_age_60No < log(1)),
            rrr_arr_2_age_below_60_mortality = 100*mean(b_age_60No < log(arr_2_or_age_below_60_mortality)),
            rrr_arr_4_age_below_60_mortality = 100*mean(b_age_60No < log(arr_4_or_age_below_60_mortality)),
            rrr_arr_6_age_below_60_mortality = 100*mean(b_age_60No < log(arr_6_or_age_below_60_mortality)),
            rrr_arr_8_age_below_60_mortality = 100*mean(b_age_60No < log(arr_8_or_age_below_60_mortality)))

# Then age above 60
Rc_age_above_60_mortality <- median(IVdat_mortality[IVdat_mortality$age_60=="Yes" &
                                               !is.na(IVdat_mortality$age_60),]$control_mortality_rate)
arr_2_Rt_age_above_60_mortality <- Rc_age_above_60_mortality - 0.02
arr_2_or_age_above_60_mortality <- (arr_2_Rt_age_above_60_mortality*(Rc_age_above_60_mortality - 1))/(Rc_age_above_60_mortality*(arr_2_Rt_age_above_60_mortality - 1))

arr_4_Rt_age_above_60_mortality <- Rc_age_above_60_mortality - 0.04
arr_4_or_age_above_60_mortality <- (arr_4_Rt_age_above_60_mortality*(Rc_age_above_60_mortality - 1))/(Rc_age_above_60_mortality*(arr_4_Rt_age_above_60_mortality - 1))

arr_6_Rt_age_above_60_mortality <- Rc_age_above_60_mortality - 0.06
arr_6_or_age_above_60_mortality <- (arr_6_Rt_age_above_60_mortality*(Rc_age_above_60_mortality - 1))/(Rc_age_above_60_mortality*(arr_6_Rt_age_above_60_mortality - 1))

arr_8_Rt_age_above_60_mortality <- Rc_age_above_60_mortality - 0.08
arr_8_or_age_above_60_mortality <- (arr_8_Rt_age_above_60_mortality*(Rc_age_above_60_mortality - 1))/(Rc_age_above_60_mortality*(arr_8_Rt_age_above_60_mortality - 1))
...
probs_age_above_60_mortality = 
  ...
  summarise(any_benefit_age_above_60_mortality = 100*mean(b_age_60Yes < log(1)),
            rrr_arr_2_age_above_60_mortality = 100*mean(b_age_60Yes < log(arr_2_or_age_above_60_mortality)),
            rrr_arr_4_age_above_60_mortality = 100*mean(b_age_60Yes < log(arr_4_or_age_above_60_mortality)),
            rrr_arr_6_age_above_60_mortality = 100*mean(b_age_60Yes < log(arr_6_or_age_above_60_mortality)),
            rrr_arr_8_age_above_60_mortality = 100*mean(b_age_60Yes < log(arr_8_or_age_above_60_mortality)))
...
arr_2_Rt_high_dose_mortality <- Rc_high_dose_mortality - 0.02
arr_2_or_high_dose_mortality <- (arr_2_Rt_high_dose_mortality*(Rc_high_dose_mortality - 1))/(Rc_high_dose_mortality*(arr_2_Rt_high_dose_mortality - 1))

arr_4_Rt_high_dose_mortality <- Rc_high_dose_mortality - 0.04
arr_4_or_high_dose_mortality <- (arr_4_Rt_high_dose_mortality*(Rc_high_dose_mortality - 1))/(Rc_high_dose_mortality*(arr_4_Rt_high_dose_mortality - 1))

arr_6_Rt_high_dose_mortality <- Rc_high_dose_mortality - 0.06
arr_6_or_high_dose_mortality <- (arr_6_Rt_high_dose_mortality*(Rc_high_dose_mortality - 1))/(Rc_high_dose_mortality*(arr_6_Rt_high_dose_mortality - 1))

arr_8_Rt_high_dose_mortality <- Rc_high_dose_mortality - 0.08
arr_8_or_high_dose_mortality <- (arr_8_Rt_high_dose_mortality*(Rc_high_dose_mortality - 1))/(Rc_high_dose_mortality*(arr_8_Rt_high_dose_mortality - 1))
...
probs_high_dose_mortality = 
  ...
  summarise(any_benefit_high_dose_mortality = 100*mean(b_dex_doseHigh < log(1)),
            rrr_arr_2_high_dose_mortality = 100*mean(b_dex_doseHigh < log(arr_2_or_high_dose_mortality)),
            rrr_arr_4_high_dose_mortality = 100*mean(b_dex_doseHigh < log(arr_4_or_high_dose_mortality)),
            rrr_arr_6_high_dose_mortality = 100*mean(b_dex_doseHigh < log(arr_6_or_high_dose_mortality)),
            rrr_arr_8_high_dose_mortality = 100*mean(b_dex_doseHigh < log(arr_8_or_high_dose_mortality)))

# Low dose
Rc_low_dose_mortality <- median(IVdat_mortality[IVdat_mortality$dex_dose=="Low" &
                                               !is.na(IVdat_mortality$dex_dose),]$control_mortality_rate)
arr_2_Rt_low_dose_mortality <- Rc_low_dose_mortality - 0.02
arr_2_or_low_dose_mortality <- (arr_2_Rt_low_dose_mortality*(Rc_low_dose_mortality - 1))/(Rc_low_dose_mortality*(arr_2_Rt_low_dose_mortality - 1))

arr_4_Rt_low_dose_mortality <- Rc_low_dose_mortality - 0.04
arr_4_or_low_dose_mortality <- (arr_4_Rt_low_dose_mortality*(Rc_low_dose_mortality - 1))/(Rc_low_dose_mortality*(arr_4_Rt_low_dose_mortality - 1))

arr_6_Rt_low_dose_mortality <- Rc_low_dose_mortality - 0.06
arr_6_or_low_dose_mortality <- (arr_6_Rt_low_dose_mortality*(Rc_low_dose_mortality - 1))/(Rc_low_dose_mortality*(arr_6_Rt_low_dose_mortality - 1))

arr_8_Rt_low_dose_mortality <- Rc_low_dose_mortality - 0.08
arr_8_or_low_dose_mortality <- (arr_8_Rt_low_dose_mortality*(Rc_low_dose_mortality - 1))/(Rc_low_dose_mortality*(arr_8_Rt_low_dose_mortality - 1))
...
probs_low_dose_mortality = 
  ...
  summarise(any_benefit_low_dose_mortality = 100*mean(b_dex_doseLow < log(1)),
            rrr_arr_2_low_dose_mortality = 100*mean(b_dex_doseLow < log(arr_2_or_low_dose_mortality)),
            rrr_arr_4_low_dose_mortality = 100*mean(b_dex_doseLow < log(arr_4_or_low_dose_mortality)),
            rrr_arr_6_low_dose_mortality = 100*mean(b_dex_doseLow < log(arr_6_or_low_dose_mortality)),
            rrr_arr_8_low_dose_mortality = 100*mean(b_dex_doseLow < log(arr_8_or_low_dose_mortality)))
...
colnames(prob_tbl2) <- c("Subgroup", "Median control group rate (%)", "Probability of any benefit (%)", "Probability of ARR = 2 (%)",
                        "Probability of ARR = 4 (%)", "Probability of ARR = 6 (%)", "Probability of ARR = 8 (%)")
...
Before we decide on this, let's focus on mean differences on 0.5, 1, and 2. @tbl-benefit-probs-secondary-cont shows the results.
...
As [Bartos et al.](https://onlinelibrary.wiley.com/doi/full/10.1002/jrsm.1594) put it:
...
Publication bias models include selection models and PET-PEESE models as methods of publication bias assessment. The publication bias adjusment prior is described in [Bartoš (2021).](https://onlinelibrary.wiley.com/doi/full/10.1002/jrsm.1594)
...
## Frequentist methods for small study effects

### Contour-enhanced funnel plot

Below in @fig-freq-funnel is the contour-enhanced funnel plot. As you can see, there is clear asymmetry to the plot, with small studies missing from the right hand side (area of no effect or harm). 
...
```{r}
#| label: fig-freq-funnel
#| fig-cap: |
#|    Contour-enhaced funnel plot for the outcome of postoperative delirium.
#| fig.height: 5
#| fig.width: 5
library(meta)
...
However, in the presence of significant heterogeneity in random effects meta-analysis, @eq-weights-re results in the relative downweighting of larger studies and upweighting of smaller studies. This means smaller studies have a greater influence on the pooled outcome, which exacerbates any impact of publication bias.

Henmi and Copas proposed that studies in a random effects meta-analysis be weighted according to fixed effect @eq-weights-fe. This means that $\tau$ is calculated and incorporated into the results but is not used to weight the studies. This approach was later popularised by Doi et al. as the 'Inverse Variance Heterogeneity Model'.

@tbl-henmi-copas shows the results.

```{r}
#| label: tbl-henmi-copas
#| tbl-cap: |
#|    Results of the Henmi and Copas method for random effects meta-analysis, where studies are weighed according using fixed effect weighting.

res_REML_hc <- rma(yi, vi, data=IVdat_del, weights = 1/vi,
                        test="knha", method = "REML")
df <- data.frame(predict(res_REML_hc, transf=exp))

df <- df %>%
  mutate(across(where(is.numeric), ~sprintf("%.2f", .)),
         confint = paste0(ci.lb, ", ", ci.ub),
         predint = paste0(pi.lb, ", ", pi.ub)) %>%
  dplyr::select(pred, confint, predint)

colnames(df) <- c("Odds ratio", "95% confidence interval", "95% prediction interval")
...
See the [bookdown selection model page](https://bookdown.org/MathiasHarrer/Doing_Meta_Analysis_in_R/pub-bias.html#selection-models) and the [metafor selection model page](https://wviechtb.github.io/metafor/reference/selmodel.html), and [Dan Quintana's blog](https://www.dsquintana.blog/how-to-perform-a-bayesian-meta-analysis-in-r/) for more details.

```{r selection_models}
df <- selmodel(res_REML,
         type = "stepfun",
         steps = c(0.025,1),
         alternative = "less")
...
df <- data.frame(predict(taf, transf=exp))
